...
export const QuotePreview = forwardRef<HTMLDivElement, QuotePreviewProps>(({
  ...
  // Calcola la data di scadenza basata sulla validità selezionata
                  ...
                  {selectedServices.map((service, idx) => {
                    ...
                    return (
                      <tr key={service.id} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} style={{ pageBreakInside: 'avoid' }}>
                        ...
                        <td className="p-2 border border-gray-200 text-[8px] text-gray-600 align-middle leading-snug">
                          {service.descrizione}
                        </td>
                        <td className="p-2 border border-gray-200 text-center font-medium align-middle">
                          {service.quantita}
                        </td>
